...
void start_scheduler(int max_batch, int workers, const std::string& impl) {
  ...
  if (max_batch <= 0) throw py::value_error("max_batch must be positive");
  if (workers <= 0) throw py::value_error("workers must be positive");
  py::gil_scoped_release release;
  std::lock_guard<std::mutex> lk(g_mutex);
  g_opts.max_batch = max_batch;
  g_opts.num_workers = workers;
  g_opts.impl = impl;
  if (g_running) return;  // already running; keep the current instance
  g_sched.reset();
  g_sched = std::make_unique<gbis::BatchScheduler>(g_opts);
  g_sched->start();
  g_running = true;
...
py::dict scheduler_stats() {
  ...
  d["throughput_jobs_per_s"] =
      wall > 0.0 ? static_cast<double>(g_sched->completed()) / wall : 0.0;
  d["wall_seconds"] = wall;
...
PYBIND11_MODULE(gpuinfer, m) {
  ...
  m.def(
      ...
      [](py::array a, py::array b) -> py::array {
        ...
        if (a.ndim() != 3) throw py::value_error("a must be 3-D (B, M, K)");
        if (b.ndim() != 3) throw py::value_error("b must be 3-D (B, K, N)");
...

        {
          ...
          CUDA_CHECK(cudaMemcpyAsync(da.data(), a_info.ptr, da.bytes(),
                                     cudaMemcpyHostToDevice, 0));
          CUDA_CHECK(cudaMemcpyAsync(db.data(), b_info.ptr, db.bytes(),
                                     cudaMemcpyHostToDevice, 0));
          gbis::gemm_tiled_batched(da.data(), db.data(), dc.data(), m_i, n_i,
                                   k_i, batch, 0);
          CUDA_CHECK(cudaMemcpyAsync(r_info.ptr, dc.data(), dc.bytes(),
  ...
  m.def(
      "submit_job",
      [](int m_dim, int n_dim, int k_dim, int batch, int priority)
          -> std::uint64_t {
        if (m_dim <= 0 || n_dim <= 0 || k_dim <= 0) {
          throw py::value_error("m, n and k must be positive");
        }
        if (batch <= 0) throw py::value_error("batch must be positive");
      ...
      py::arg("m"), py::arg("n"), py::arg("k"), py::arg("batch") = 1,
      py::arg("priority") = 0);
  ...
  py::class_<SchedulerHandle>(m, "Scheduler")
      .def(py::init([](int max_batch, int workers, const std::string& impl) {
           ...
           py::arg("max_batch") = 8, py::arg("workers") = 2,
           py::arg("impl") = "tiled")
      .def("start",
           [](SchedulerHandle& h) {
             start_scheduler(h.max_batch, h.workers, h.impl);
           })
